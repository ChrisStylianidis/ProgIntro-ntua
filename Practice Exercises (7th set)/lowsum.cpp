...
int main()
{
    ...
    if (flag)
    { // an mpei sto if simenei oti einai oloi negative
        checkme = false;
        minn = a[n - 1] + a[n - 2];
    }
    if (pos)
    ...
    if (checkme)
    { // vazo mia bool checkme etsi oste an einai oloi thetikoi i arnitikoi na min mpei mesa stin diadikasia afou exo idi ipologisei ta min
        do
        {
            right = a[pright];
            left = a[pleft];
